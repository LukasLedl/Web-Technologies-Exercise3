//ich hatte Hilfe von Glory Adam Temu-Nduwayezu
...
class ShoppingCart {
    ...
    add(book) {
        ...
        li.append("span", book.title, " quantity ");
        li.append("span", book.quantity, " subtotal ");
        let totalPrice = book.quantity * book.price;
        totalPrice = this.round(totalPrice);
        li.append(totalPrice, " €");

        li.id = book.cartId;
        const unordedList = document.querySelector("aside ul");
        unordedList.append(li);
...
class BookStore {
    ...
    addCategoryToDOM(category) {
        let section = document.createElement("section"); // Create an empty <section> element
        section.id = category.id;
        let h1 = document.createElement("h1"); // Create an empty <h1> element
    ...
    addBookToDOM(category, book) {
        ...
        pQuantity.append(label, Select, Button);

        let Price = document.createElement("p");
        Price.textContent = "Price:" + book.price + "€";

        let Dis = document.createElement("p");
        Dis.textContent = book.description;

        article.append(h3, Img, pQuantity, Price, Dis);
        document.querySelector("main").append(document.querySelector("main > a[href]"));
